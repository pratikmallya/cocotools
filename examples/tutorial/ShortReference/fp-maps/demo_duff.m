...
% contpars = {'a'};
% contpars = {'a' 'test_LP' 'test_BP'};
contpars = {'a' 'POSym' 'PRCnd'};

bd1 = coco(opts,'1' , 'curve','isol','sol',f,[0;0],[1;0.2], contpars,[0 4]);
hold on
plot_bd(bd1)
hold off
drawnow
...
% opts = coco_set(opts, 'curve', 'switch', 3);

bd2 = coco(opts,'2' , 'curve','BP','sol','1',labs(1) , contpars,[0 4]);
hold on
plot_bd(bd2)
hold off
...
opts = coco_set(opts, 'curve', 'switch', 2);
...
for i=1:7
  ...
  bd3 = coco(opts,run , 'curve','PD','sol',rrun,labs(1) , contpars,[0 4]);
  hold on
  plot_bd(bd3)
  hold off
end
