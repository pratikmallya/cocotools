...
p0 = [ 0.5 ; 0.5/a ; a ; b ];
z0 = 0.5/b;
axis([-1 1 -1 1 -1/b 1/b]); view([135 0])
...
opts = coco_set(opts, 'cont'    , 'ItMX'    , 10000);

opts = coco_set(opts, 'cont'    , 'NPR'     , 100 );
opts = coco_set(opts, 'cont'    , 'NSV'     , 1000);
opts = coco_set(opts, 'cont'    , 'LogLevel', 4);

opts = coco_set(opts, 'cont'    , 'al_max'   , 10     );
opts = coco_set(opts, 'cont'    , 'h0'       , 0.25/b );
opts = coco_set(opts, 'cont'    , 'h_max'    , 1.00/b );
opts = coco_set(opts, 'cont'    , 'h_min'    , 0.10/b );
