classdef atlas2_x < AtlasBase
  ...
  methods % interface methods
    ...
    function [prob atlas cseg flush] = add_chart(atlas, prob, cseg)
      ...
      if ~atlas.isclose(cseg.ptlist{1}, cseg.ptlist{end})
  ...
  methods( Access = private ) % private utility functions
    ...
    function [atlas last_chart] = merge_into_atlas(atlas, ptlist)
      ...
      for i=1:numel(ptlist)
        ...
        idx           = cellfun(@(x) atlas.isneighbour(chart, x), bd_charts);
  ...
  methods (Static,Access=public)
    ...
    function plot_debug(atlas, ix,iy,iz)
      atlas.print_nb(atlas.charts);
      fprintf('boundary =');
      fprintf(' %d', atlas.boundary);
      fprintf('\n');
      [az el] = view;
      atlas.plot_voronoi(atlas.charts, ix,iy,iz);
      view([az el]);
      drawnow
    end
    
    function print_nb(charts)
      fprintf('***********************\n');
      for k=1:numel(charts)
        chart = charts{k};
        fprintf('nb(%d) =', chart.id);
        fprintf(' %d', chart.nb);
        fprintf('\n');
      end
    end
    
    function plot_voronoi(charts, ix, iy, iz)
      cla
      hold on
      for k=1:numel(charts)
        chart = charts{k};
        x     = chart.x;
        text(x(ix), x(iy), x(iz), sprintf('%d', chart.id), ...
        ...
        for l=1:size(chart.s,2)-1;
          vx1 = x+chart.v(l)*(chart.TS*chart.s(:,l));
          vx2 = x+chart.v(l+1)*(chart.TS*chart.s(:,l+1));
          plot3([vx1(ix) vx2(ix)], [vx1(iy) vx2(iy)], [vx1(iz) vx2(iz)], style);
        end
        vx1 = x+chart.v(end)*(chart.TS*chart.s(:,end));
        vx2 = x+chart.v(1)*(chart.TS*chart.s(:,1));
        plot3([vx1(ix) vx2(ix)], [vx1(iy) vx2(iy)], [vx1(iz) vx2(iz)], style);
      end
      hold off
    end
    ...
    function [tri X C] = trisurf(charts, ix, iy, iz)
      if nargin>=4
        f = @(x) x([ix iy iz])';
      else
        f = @(x) x';
