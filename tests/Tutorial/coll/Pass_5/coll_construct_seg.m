...
err   = cdata.err(1);
maps2 = maps;
if err>coll.TOLINC && maps.NTST~=NTSTi
  data.coll.NTST = NTSTi;
  maps2 = coll_maps(int, NTSTi, maps.pdim);
elseif err<coll.TOLDEC && maps.NTST~=NTSTd
  data.coll.NTST = NTSTd;
  maps2 = coll_maps(int, NTSTd, maps.pdim);
end
xtr(maps.xtrend) = maps2.xtr(maps2.xtrend);
th = linspace(0, F(end), maps2.NTST+1);
tt = interp1(F, t*(maps2.NTST/t(end)), th, 'cubic');

mesh2 = coll_mesh(int, maps2, tt);
