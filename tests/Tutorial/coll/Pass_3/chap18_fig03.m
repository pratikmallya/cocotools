...
db.plot_create('chap18_fig03a', mfilename('fullpath'));
...
db.textarrow(sol3.x(550,1), sol3.x(550,2), 1, 'NTST=150, NCOL=5', 'tr', 'lab');
...
db.plot_create('chap18_fig03b', mfilename('fullpath'));
db.paper_size([16 4.5]);

db.plot(sol3.t/sol3.t(end), sol3.x(:,2), 'numdata'); %, 'line1g5');
db.plot(sol2.t/sol2.t(end), sol2.x(:,2), 'numdata'); %, 'line1g7');
...
db.textarrow(sol3.t(200)/sol3.t(end), sol3.x(200,2), 2, 'NTST=150, NCOL=5', 'tr', 'lab');
...
db.plot_create('chap18_fig03c', mfilename('fullpath'));
db.paper_size([16 4.5]);
...
db.plot(p,err, 'numdata');
db.textarrow(p(end-2), err(end-2), 1, 'NTST=20, NCOL=4', 'r', 'lab');

p   = coco_bd_col(bd3, 'eps');
err = coco_bd_col(bd3, 'po.seg.coll.err');
db.plot(p,err, 'numdata');
db.textarrow(p(end-12), err(end-12), 1, 'NTST=150, NCOL=5', 'tl', 'lab');
