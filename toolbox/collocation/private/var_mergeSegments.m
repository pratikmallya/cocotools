function [opts coll x0] = var_mergeSegments(opts, coll, x0)
...
mcolidx   = [];          % indices for reshaping M at collocation points
...
dmcoltr   = -ones(midxoff,1); % translation table for column-indices Mcol->xcol
...
mcolidxoff = 0;          % offset of indices for reshaping M
...
for segnum = 1:length(segs)
  ...
  seg_mcolidx = reshape(segs(segnum).dmcolidx, segs(segnum).mshape);
  mcolidx  = [mcolidx mcolidxoff + seg_mcolidx]; %#ok<AGROW>
	dmrows   = [dmrows  ; max([0 ; dmrows ]) + segs(segnum).dmrows ]; %#ok<AGROW>
	dmcols   = [dmcols  ; max([0 ; dmcols ]) + segs(segnum).dmcols ]; %#ok<AGROW>
  dmcoltr  = [dmcoltr ; repmat(segs(segnum).alidx'+segs(segnum).varidxoff, dim, 1)]; %#ok<AGROW>
...
coll.mcolidx  = mcolidx;
coll.dmrows   = dmrows;
coll.dmcols   = dmcols;
coll.dmcoltr  = dmcoltr;
coll.mshape   = mshape;
